...
export class TivotJsonParserService {
  parse(rawContent: string): TivotAssistantPayload | null {
    const jsonSource = this.extractJsonObject(rawContent)
  ...
  private extractJsonObject(rawContent: string): string | null {
    ...
    return this.findBalancedJsonObject(source)
  }

  private findBalancedJsonObject(source: string): string | null {
    let depth = 0
    let start = -1
    let inString = false
    let escaped = false
